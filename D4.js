...
const crazyDiff = function(num,numP= 19 ){
    const diff = Math.abs(num - numP)
    if(diff < numP){
        return diff *3
    }else{
        return diff
...
const epify = function(string){
    const stringE = string.indexOf("EPICODE")
    if(stringE >= 0){
        return string.push("EPICODE")
    }else{
        return string
    }
}
...
const reverseString = function (parInvert){
    return parInvert.split("").reverse("").join("")
}
